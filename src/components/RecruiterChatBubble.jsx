...
const SUGGESTIONS = [
  ...
  "Which pipeline needs attention?",
  'Compare my top applicants for a role',
]
...
export default function RecruiterChatBubble() {
  const { user } = useAuth()
  ...
  const { messages, loading, sending, loadMessages, sendMessage } = useChat(user?.id, 'recruiter-chat')
  ...
  if (location.pathname === '/recruiter/chat') return null
      ...
      <div className={`hchat-drawer ${isOpen ? 'open' : 'closed'}`}>
        <div className="hchat-head">
          ...
          <div className="hchat-head-info">
            <strong>AI Recruiting Assistant</strong>
          ...
          <div className="hchat-head-btns">
            <button
              ...
              onClick={() => { setIsOpen(false); navigate('/recruiter/chat') }}
            >
        ...
        <div className="hchat-messages">
          {loading ? (
            ...
            <div className="hchat-empty">
              ...
              <p>I have live access to all your clients, roles, and candidate scores.</p>
              <div className="hchat-suggestions">
                {SUGGESTIONS.map(s => (
                  <button key={s} className="hchat-suggestion" onClick={() => { setInput(s); inputRef.current?.focus() }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            messages.map(msg => (
              <div key={msg.id} className={`hchat-row ${msg.role}`}>
                ...
                <div className="hchat-bubble-msg" dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.message) }} />
              </div>
            ))
          )}
          {sending && (
        ...
        <div className="hchat-input-area">
          <textarea
            ...
            placeholder="Ask about candidates or pipeline…"
            rows={1}
          />
          <button className="hchat-send" onClick={handleSend} disabled={!input.trim() || sending}>
