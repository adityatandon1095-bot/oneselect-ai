...
export default function RecruiterDashboard() {
  ...
  async function load() {
    const { data: jobs } = await supabase.from('jobs').select('id, title, status, created_at').eq('recruiter_id', user.id).order('created_at', { ascending: false })
            ...
            {[
              ...
              return (
                <div key={step.label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                  ...
                  {i > 0 && (
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: step.color, marginTop: 4 }}>{pct}%</div>
                  )}
                  {/* Bar */}
